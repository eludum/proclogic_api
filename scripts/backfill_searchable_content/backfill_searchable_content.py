...
def _load_batch(session, offset: int, batch_size: int, only_missing: bool) -> List[Publication]:
...
def backfill(batch_size: int = BATCH_SIZE, limit: Optional[int] = None, only_missing: bool = True) -> None:
    ...
    while True:
        ...
        with get_session() as session:
            ...
            batch = _load_batch(
                session, 0 if only_missing else offset, size, only_missing
            )
            if not batch:
                break
            ...
            try:
                ...
                session.rollback()
                # Skip past the offending batch rather than looping on it.
                offset += len(batch)
                processed += len(batch)
                continue
    ...
    logger.info(
        "Done. Processed %d publication(s), wrote %d, in %.1fs",
        processed,
        written,
        time.time() - started,
    )
