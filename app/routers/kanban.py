...
async def get_kanban_board(auth_user: AuthUser = Depends(get_auth_user)):
    ...
    with get_session() as session:
        ...
        # Get the board data
        statuses, publications_by_status = crud_kanban.get_kanban_board(company.vat_number, session)
...
async def create_kanban_status(
    ...
    with get_session() as session:
        ...
        if not new_status:
            raise HTTPException(status_code=500, detail="Failed to create Kanban status")
...
async def add_publication_to_kanban(
    ...
    with get_session() as session:
        ...
        if not pub_status:
            raise HTTPException(
                status_code=404, 
                detail="Publication or status not found"
            )
...
async def get_publication_kanban_status(
    ...
    with get_session() as session:
        ...
        if not pub_status:
            raise HTTPException(
                status_code=404, 
                detail="Publication not found in Kanban board"
            )
...
async def move_publication(
    move_request: MovePublicationRequest,
    auth_user: AuthUser = Depends(get_auth_user)
):
...
async def initialize_kanban_board(auth_user: AuthUser = Depends(get_auth_user)):
    ...
    with get_session() as session:
        ...
        if not success:
            raise HTTPException(
                status_code=500, 
                detail="Failed to initialize Kanban board"
            )
