...
def get_related_publications(
    ...
    """
    Find related publications using optimized database-level similarity calculation.
    ...
    similarity_score = (
        ...
        case(
            ...
            (func.substring(Publication.cpv_main_code_code, 1, 4) ==
             publication.cpv_main_code.code[:4], 30),
            (func.substring(Publication.cpv_main_code_code, 1, 2) ==
             publication.cpv_main_code.code[:2], 25),
            else_=0
        ) +
        # Same organization (20 points)
        case(
            (Publication.organisation_id == publication.organisation_id, 20),
            else_=0
        ) +
        # Geographic overlap (15 points) - using array overlap operator
        case(
            (Publication.nuts_codes.op('&&')(publication.nuts_codes), 15),
            else_=0
        ) +
        ...
        case(
            (and_(
                ...
                publication.estimated_value > 0,
                func.least(Publication.estimated_value,
                          publication.estimated_value) /
                func.greatest(Publication.estimated_value,
                             publication.estimated_value) > 0.5
            ), 10),
            else_=0
        )
    ...
    base_query = (
        ...
        .filter(Publication.publication_workspace_id !=
    ...
    results = (
        base_query
        .filter(similarity_score > 10)  # Minimum threshold
    ...
    for pub, score in results:
        ...
        if (pub.extracted_keywords and publication.extracted_keywords and
            set(pub.extracted_keywords) & set(publication.extracted_keywords)):
            common = (set(pub.extracted_keywords) &
                     set(publication.extracted_keywords))
            reasons.append(f"Common keywords: {', '.join(list(common)[:3])}")

        if (pub.estimated_value and publication.estimated_value and
            pub.estimated_value > 0 and publication.estimated_value > 0 and
            min(pub.estimated_value, publication.estimated_value) /
            max(pub.estimated_value, publication.estimated_value) > 0.5):
