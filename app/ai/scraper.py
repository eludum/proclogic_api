...
async def scrape_company_website(
    ...
    try:
        ...
        async with httpx.AsyncClient(
            timeout=15.0, transport=transport, follow_redirects=True
        ) as http_client:
            ...
            try:
                ...
                if website_url.startswith("https://"):
                    ...
                    try:
                        ...
                        if response.status_code != 200:
                            logging.error(
                                f"Failed to fetch website with HTTP: {response.status_code}"
                            )
                            return None
            ...
            logging.info(
                f"Successfully scraped main page and {len([r for r in results if r])}/{len(additional_pages)} additional pages"
            )
            ...
            if len(combined_content) > max_chars:
                combined_content = (
                    combined_content[:max_chars] + "...[content truncated]"
                )
